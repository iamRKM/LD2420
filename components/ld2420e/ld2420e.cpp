...
namespace esphome
{
  namespace ld2420e
  {
    ...
    static const char *const TAG = "ld2420e";

    float LD2420EComponent::get_setup_priority() const { return setup_priority::BUS; }

    void LD2420EComponent::dump_config()
    {
      ESP_LOGCONFIG(TAG, "LD2420E:");
      ESP_LOGCONFIG(TAG, "  Firmware Version : %7s", this->ld2420e_firmware_ver_);
      ESP_LOGCONFIG(TAG, "LD2420E Number:");
#ifdef USE_NUMBER
      ...
      for (uint8_t gate = 0; gate < LD2420E_TOTAL_GATES; gate++)
      {
...
#endif
      ESP_LOGCONFIG(TAG, "LD2420E Select:");
      LOG_SELECT(TAG, "  Operating Mode", this->operating_selector_);
      if (this->get_firmware_int_(ld2420e_firmware_ver_) < CALIBRATE_VERSION_MIN)
      {
        ESP_LOGW(TAG, "LD2420E Firmware Version %s and older are only supported in Simple Mode", ld2420e_firmware_ver_);
      }
    }

    uint8_t LD2420EComponent::calc_checksum(void *data, size_t size)
    {
      uint8_t checksum = 0;
    ...
    int LD2420EComponent::get_firmware_int_(const char *version_string)
    {
    ...
    void LD2420EComponent::setup()
    {
      ESP_LOGCONFIG(TAG, "Setting up LD2420E...");
      if (this->set_config_mode(true) == LD2420E_ERROR_TIMEOUT)
      {
        ESP_LOGE(TAG, "LD2420E module has failed to respond, check baud rate and serial connections.");
        this->mark_failed();
        return;
      }
      ...
      const char *pfw = this->ld2420e_firmware_ver_;
      ...
      if (get_firmware_int_(ld2420e_firmware_ver_) < CALIBRATE_VERSION_MIN)
      {
        this->set_operating_mode(OP_SIMPLE_MODE_STRING);
        this->operating_selector_->publish_state(OP_SIMPLE_MODE_STRING);
        this->set_mode_(CMD_SYSTEM_MODE_SIMPLE);
        ESP_LOGW(TAG, "LD2420E Frimware Version %s and older are only supported in Simple Mode", ld2420e_firmware_ver_);
      }
      else
      {
      ...
      ESP_LOGCONFIG(TAG, "LD2420E setup complete.");
    }

    void LD2420EComponent::apply_config_action()
    {
      ...
      ESP_LOGCONFIG(TAG, "Reconfiguring LD2420E...");
      if (this->set_config_mode(true) == LD2420E_ERROR_TIMEOUT)
      {
        ESP_LOGE(TAG, "LD2420E module has failed to respond, check baud rate and serial connections.");
        this->mark_failed();
...
#endif
      this->set_system_mode(this->system_mode_);
      this->set_config_mode(false); // Disable config mode to save new values in LD2420E nvm
      this->set_operating_mode(OP_NORMAL_MODE_STRING);
      ESP_LOGCONFIG(TAG, "LD2420E reconfig complete.");
    }

    void LD2420EComponent::factory_reset_action()
    {
...
#endif
      ESP_LOGCONFIG(TAG, "LD2420E factory reset complete.");
    }

    void LD2420EComponent::restart_module_action()
    {
      ESP_LOGCONFIG(TAG, "Restarting LD2420E module...");
    ...
    this->set_config_mode(false); });
      ESP_LOGCONFIG(TAG, "LD2420E Restarted.");
    }

    void LD2420EComponent::revert_config_action()
    {
      memcpy(&this->new_config, &this->current_config, sizeof(this->current_config));
#ifdef USE_NUMBER
      this->init_gate_config_numbers();
    ...
    void LD2420EComponent::update_radar_data(uint16_t const *gate_energy, uint8_t sample_number)
    {
      for (uint8_t gate = 0; gate < LD2420E_TOTAL_GATES; ++gate)
      {
    ...
    void LD2420EComponent::auto_calibrate_sensitivity()
    {
    ...
    void LD2420EComponent::report_gate_data()
    {
      for (uint8_t gate = 0; gate < LD2420E_TOTAL_GATES; ++gate)
      {
        // Output results
    ...
    void LD2420EComponent::set_operating_mode(const std::string &state)
    {
    ...
    void LD2420EComponent::readline_(int rx_data, uint8_t *buffer, int len)
    {
      static int pos = 0;
    ...
    void LD2420EComponent::handle_energy_mode_(uint8_t *buffer, int len)
    {
    ...
    void LD2420EComponent::handle_simple_mode_(const uint8_t *inbuf, int len)
    {
    ...
    void LD2420EComponent::handle_ack_data_(uint8_t *buffer, int len)
    {
      ...
      {
        ESP_LOGW(TAG, "LD2420E reply - received command reply frame is corrupt, length exceeds %d bytes.", CMD_MAX_BYTES);
        return;
      }
      else if (this->cmd_reply_.length < 2)
      {
        ESP_LOGW(TAG, "LD2420E reply - received command frame is corrupt, length is less than 2 bytes.");
        return;
      }
      ...
      case (CMD_ENABLE_CONF):
        ESP_LOGD(TAG, "LD2420E reply - set config enable: CMD = %2X %s", CMD_ENABLE_CONF, result);
        break;
      case (CMD_DISABLE_CONF):
        ESP_LOGD(TAG, "LD2420E reply - set config disable: CMD = %2X %s", CMD_DISABLE_CONF, result);
        break;
      case (CMD_READ_REGISTER):
        ESP_LOGD(TAG, "LD2420E reply - read register: CMD = %2X %s", CMD_READ_REGISTER, result);
      ...
      case (CMD_WRITE_REGISTER):
        ESP_LOGD(TAG, "LD2420E reply - write register: CMD = %2X %s", CMD_WRITE_REGISTER, result);
        break;
      case (CMD_WRITE_ABD_PARAM):
        ESP_LOGD(TAG, "LD2420E reply - write gate parameter(s): %2X %s", CMD_WRITE_ABD_PARAM, result);
        break;
      case (CMD_READ_ABD_PARAM):
        ESP_LOGD(TAG, "LD2420E reply - read gate parameter(s): %2X %s", CMD_READ_ABD_PARAM, result);
      ...
      case (CMD_WRITE_SYS_PARAM):
        ESP_LOGD(TAG, "LD2420E reply - set system parameter(s): %2X %s", CMD_WRITE_SYS_PARAM, result);
        break;
      case (CMD_READ_VERSION):
        memcpy(this->ld2420e_firmware_ver_, &buffer[12], buffer[10]);
        ESP_LOGD(TAG, "LD2420E reply - module firmware version: %7s %s", this->ld2420e_firmware_ver_, result);
        break;
      default:
        break;
      }
    ...
    int LD2420EComponent::send_cmd_from_array(CmdFrameT frame)
    {
      uint8_t error = 0;
    ...
    uint8_t LD2420EComponent::set_config_mode(bool enable)
    {
      CmdFrameT cmd_frame;
    ...
    void LD2420EComponent::send_module_restart() { this->ld2420e_restart(); }

    void LD2420EComponent::ld2420e_restart()
    {
      CmdFrameT cmd_frame;
      cmd_frame.data_length = 0;
      cmd_frame.header = CMD_FRAME_HEADER;
    ...
    void LD2420EComponent::get_reg_value_(uint16_t reg)
    {
      CmdFrameT cmd_frame;
      cmd_frame.data_length = 0;
      cmd_frame.header = CMD_FRAME_HEADER;
    ...
    void LD2420EComponent::set_reg_value(uint16_t reg, uint16_t value)
    {
      CmdFrameT cmd_frame;
      cmd_frame.data_length = 0;
      cmd_frame.header = CMD_FRAME_HEADER;
    ...
    void LD2420EComponent::handle_cmd_error(uint8_t error) { ESP_LOGI(TAG, "Command failed: %s", ERR_MESSAGE[error]); }

    int LD2420EComponent::get_gate_threshold_(uint8_t gate)
    {
      uint8_t error;
      CmdFrameT cmd_frame;
      cmd_frame.data_length = 0;
    ...
    int LD2420EComponent::get_min_max_distances_timeout_()
    {
      uint8_t error;
      CmdFrameT cmd_frame;
      cmd_frame.data_length = 0;
    ...
    void LD2420EComponent::set_system_mode(uint16_t mode)
    {
      CmdFrameT cmd_frame;
    ...
    void LD2420EComponent::get_firmware_version_()
    {
      CmdFrameT cmd_frame;
      cmd_frame.data_length = 0;
      cmd_frame.header = CMD_FRAME_HEADER;
    ...
    void LD2420EComponent::set_min_max_distances_timeout(uint32_t max_gate_distance, uint32_t min_gate_distance, // NOLINT
                                                         uint32_t timeout)
    {
    ...
    void LD2420EComponent::set_gate_threshold(uint8_t gate)
    {
...
#ifdef USE_NUMBER
    void LD2420EComponent::init_gate_config_numbers()
    {
    ...
    void LD2420EComponent::refresh_gate_config_numbers()
    {
